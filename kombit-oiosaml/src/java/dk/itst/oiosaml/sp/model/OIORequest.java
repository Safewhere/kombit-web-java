    ...
    protected class Encoder extends HTTPRedirectDeflateEncoder {
		public String buildRedirectURL(Credential signingCredential, String relayState) throws MessageEncodingException {
			...
			messageContext.setOutboundSAMLMessageSigningCredential(signingCredential);
                        messageContext.setSignatureMethod(SAMLConfigurationFactory.getConfiguration().getSystemConfiguration().getString(Constants.SIGNON_SIGNATURE_METHOD, SignatureConstants.ALGO_ID_SIGNATURE_RSA_SHA256));
                        messageContext.setDigestMethod(SAMLConfigurationFactory.getConfiguration().getSystemConfiguration().getString(Constants.SIGNON_DIGEST_METHOD));
